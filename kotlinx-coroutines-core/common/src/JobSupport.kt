...
public open class JobSupport constructor(active: Boolean) : Job, ChildJob, ParentJob {
    init {
        /**
         * Unfortunately, the coroutine context is not available at this point - the instance is only partially initialized,
         * and constructors that initialize coroutineContext are not yet called, so a cast to CoroutineScope won't work.
         * And we'd very much like to retrieve the context information from the associated coroutine scope.
         * Instead, we have to collect it at completion/cancellation probes.
         */
